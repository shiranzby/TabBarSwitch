# TabBarSwitch v2.2 (Chromium 浏览器标签栏滚轮切换工具)
...
*   **完美视觉体验** (v2.1 新增)：使用 `Ctrl+PgUp/PgDn` 替代 `Ctrl+Tab`，彻底解决了后台切换时地址栏出现黑色焦点框闪烁的问题。
*   **极致流畅** (v2.2 新增)：采用 `AttachThreadInput` 技术统一了后台控制逻辑，极大地提升了 Chrome 多窗口之间切换的流畅度，消除了旧版本中的焦点抢占问题。

## ⚙️ 配置说明

脚本顶部包含配置区域，你可以根据需要修改 `TabBarSwitch_v2.2.ahk` 文件：
...
1.  **直接使用（无需安装环境）**：
    *   下载 `.exe` 文件（如 `TabBarSwitch_v2.2.exe`）。
    *   **无需安装** AutoHotkey，双击即可直接运行。
    *   如果你希望完全隐藏托盘图标（静默运行），请下载 `TabBarSwitch_notrayicon_v2.2.exe`。

2.  **修改源码（需安装环境）**：
    *   下载 `.ahk` 文件（如 `TabBarSwitch_v2.2.ahk`）。
...
3.  **触发器**：新建 -> 开始任务选择“登录时”。
4.  **操作**：新建 -> 程序或脚本：
    *   **如果是 .exe 文件**：直接选择你的 `.exe` 文件路径。
    *   **如果是 .ahk 文件**：选择 `AutoHotkey64.exe` 路径，并在“添加参数”中填入脚本的完整路径。
5.  **条件**：取消勾选“只有在计算机使用交流电源时才启动此任务”（防止笔记本不插电不启动）。
