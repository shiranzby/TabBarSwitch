...
3.  将 `TabBarSwitch_v2.2.ahk` 的**快捷方式**（右键 -> 创建快捷方式）复制到这个文件夹中。
...
3.  **常规**：勾选“使用最高权限运行”。
4.  **触发器**：新建 -> 开始任务选择“登录时”。
5.  **操作**：新建 -> 程序或脚本选择你的 `AutoHotkey64.exe` 路径，在“添加参数”中填入脚本的完整路径（例如 `"D:\Tools\TabBarSwitch_v2.2.ahk"`）。
6.  **条件**：取消勾选“只有在计算机使用交流电源时才启动此任务”（防止笔记本不插电不启动）。

## 📦 安装与使用

1.  确保已安装 [AutoHotkey v2](https://www.autohotkey.com/)。
2.  下载本项目中的 `TabBarSwitch_v2.2.ahk`。
3.  双击脚本文件运行即可。
